        ...
        while True:
            ...
            if a % b == 0 and b % a == 0:
                ...
                if g == 1 or g == -1:
                    ...
                    u_a, facs_a = _decompose_relatively_int_prime([(g, k_a + k_b), (a // g, k_a)])
                    u *= u_a
                    facs = facs + facs_a
                    facs[i] = facs.pop()
...
def _adj_decompose_int(n):
    ...
    if n == 0 or n == 1:
        ...
        if p is None:
            ...
            _, facs = _decompose_relatively_int_prime([(p, 1), (n // p, 1)])
            t = ZOmega.fromInt(1)
            for (p, k) in facs:
        ...
        while True:
            ...
            if ZRootTwo.sim(a, b):
                ...
                if ZRootTwo.sim(g, 1):
                    ...
                    u_a, facs_a = _decompose_relatively_zomega_prime([(g, k_a + k_b), (a // g, k_a)])
                    u *= u_a
                    facs = facs + facs_a
                    facs[i] = facs.pop()
                    stack.append((b // g, k_b))
...
def _adj_decompose_selfcoprime(xi):
    ...
    if n == 0 or n == 1:
        ...
        if p is None:
            ...
            _, facs = _decompose_relatively_zomega_prime([(eta, 1), (xi // eta, 1)])
            t = ZOmega.fromInt(1)
            for (eta, k) in facs:
...
def _diophantine(xi):
    ...
    if t == NO_SOLUTION:
        ...
        xi_associate = ZRootTwo.fromZOmega(t.conj * t)
        u = xi // xi_associate
        v = u.sqrt()
        if v is None:
