...
def _shift_ellipse_pair(ellipse_pair, n):
    ...
    ellipse_pair.A.a *= lambda_inv_n.toReal
    ellipse_pair.A.d *= lambda_n.toReal
    ellipse_pair.B.a *= lambda_n.toReal
    ellipse_pair.B.d *= lambda_inv_n.toReal
    if n & 1:
...
def _step_lemma(ellipse_pair, opG_l, opG_r, verbose=False):
    ...
    if B.b < 0:
        ...
        n = round(log(ellipse_pair.bias) / log(LAMBDA.toReal) / 8)
        ...
        n = round(log(ellipse_pair.bias) / log(LAMBDA.toReal) / 4)
